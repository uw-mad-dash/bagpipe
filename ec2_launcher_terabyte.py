...
def return_args_download_avazu(private_ip_trainers):
    run_args_download_movielen = [
        {
            "cmd": "aws s3 cp s3://recommendation-data-bagpipe/terabyte_sample.npz ./ && aws s3 cp s3://recommendation-data-bagpipe/criteo_emb_info ./"
        }
        for i in range(len(private_ip_trainers))
    ]
...
def run_large_scale():
    ...
    ) = launch_instances_spot(launch_cfg)
    ...
    if False:
        ...
        log_file_name = "avazu_bagpipe_prefetch_16384_{}_machines_2000_iter".format(
            len(private_ip_trainers)
        )
        emb_info_file = "/home/ubuntu/avazu_emb_info"
    ...
    if True:
        # ======================Run terabyte CL ==================
        log_file_name = "terabyte_hybrid_cpu_gpu_final_run_batch_size_16384_{}_machine_original_dlrm_2000_iters.log".format(
            len(private_ip_trainers)
        )
