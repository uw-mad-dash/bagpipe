...
python examples/trainer_wdn_main.py --worker-id $worker_id --lookahead-value 1 --world-size $world_size --cache-size 500000 --master-ip $master_ip --dist-worker-id $dist_worker_id --world-size-trainers $world_size_trainers --dist-master-ip $dist_master_ip --device cuda:0 --logging-prefix $logging_prefix --dist-backend gloo --stop-iter $stop_iter --s3 2>&1 | tee out
