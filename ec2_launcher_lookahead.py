...
def return_args_trainers_dlrm(
    private_ip_trainers, private_ip_oracle_cacher, log_file_name, num_iters, cache_size, lookahead
):
    """
    ...
    run_args_trainers = [
        {
            "cmd": "rm -rf bagpipe && git clone git@github.com:iidsample/bagpipe.git && cd bagpipe && bash run_trainers_dlrm_lookahead.sh {} {} {} {} {} {} {} {} {}".format(
                i + 2,
...
def return_args_oracle_server_dlrm(
    private_ip_trainers, private_ip_oracle_cacher, batch_size, cache_size, lookahead
):
    """
    ...
    run_args_oracle_cacher = [
        {
            "cmd": "rm -rf bagpipe && git clone git@github.com:iidsample/bagpipe.git && cd bagpipe && bash run_oracle_server_dlrm_lookahead.sh {} {} {} {} {} {} {}".format(
                0,
                (len(private_ip_trainers) + 2),
                private_ip_oracle_cacher,
                len(private_ip_trainers),
...
run_args_oc_warmnup = [
    {
        "cmd": "aws s3 cp s3://recommendation-data-bagpipe/kaggle_criteo_info ./ && aws s3 cp s3://recommendation-data-bagpipe/kaggle_16 ./kaggle_16 --recursive && aws s3 cp s3://recommendation-data-bagpipe/emb_table_info.txt ./"
    }
]
...
def run_large_scale():
    ...
    batch_size = 16384
    kaggle_size = [7000000, 4000000, 4000000, 4000000, 4000000, 4000000, 4000000]
    lookahead = [300, 200, 100, 80, 60, 40, 20]
    iterations = 2000
    for i in range(6):
        # ========Launching Bagpipe run 1========================================
        cache_size = kaggle_size[i]
        lookahead_v = lookahead[i]
        log_file_name = "run_dlrm_{}_num_machines_{}_run_lookahead_{}".format(
            len(private_ip_trainers), batch_size, lookahead_v
        )
        run_args_trainers = return_args_trainers_dlrm(
            private_ip_trainers, private_ip_oracle_cacher, log_file_name, iterations, cache_size, lookahead_v
        )
        ...
        run_args_oracle_cacher = return_args_oracle_server_dlrm(
            private_ip_trainers, private_ip_oracle_cacher, batch_size, cache_size, lookahead_v
        )
